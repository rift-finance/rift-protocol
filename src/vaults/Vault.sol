    ...
    VaultStorage
{
    ...
    function depositToken0(uint256 _amount)
        external
        payable
        override
        whenNotPaused
    ...
    function depositToken1(uint256 _amount)
        external
        override
        whenNotPaused
        nonReentrant
    ...
    function _depositAccounting(
        ...
        assetData.balanceDay0[msg.sender] = __updateDepositRequests(
            assetData,
            currEpoch,
            _depositAmount
        );
    ...
    function withdrawToken0(uint256 _amount)
        external
        override
        whenNotPaused
        nonReentrant
    ...
    function withdrawToken1(uint256 _amount)
        external
        override
        whenNotPaused
        nonReentrant
    ...
    function _withdrawAccounting(
        ...
        uint256 userBalanceDay0 = __updateDepositRequests(
            assetData,
            currEpoch,
            0
        );
        ...
        if (req.amount > 0 && req.epoch < currEpoch) {
            ...
            assetData.claimable[msg.sender] +=
                (req.amount * assetData.epochToRate[req.epoch]) /
                RAY;
            req.amount = 0;
        }
        ...
        unchecked {
            assetData.balanceDay0[msg.sender] =
                userBalanceDay0 -
                _withdrawAmountDay0;
        }

        // Add it to their withdraw request and log the epoch
        assetData.withdrawRequests[msg.sender].amount =
            _withdrawAmountDay0 +
            req.amount;
    ...
    /// @param assetCode code for the type of asset (either `TOKEN0` or `TOKEN1`)
    function _claimAccounting(AssetData storage assetData, bytes32 assetCode)
        private
        returns (uint256 _claim)
    {
        ...
        if (withdrawEpoch < currEpoch) {
            ...
            if (withdrawAmountDay0 > 0) {
                ...
                claimable +=
                    (withdrawAmountDay0 *
                        assetData.epochToRate[withdrawEpoch]) /
                    RAY;
            }
        }
        ...
        )
    {
        ...
        if (withdrawReq.amount > 0) {
            ...
            if (withdrawReq.epoch < currEpoch) {
                claimable +=
                    (withdrawReq.amount *
                        assetData.epochToRate[withdrawReq.epoch]) /
                    RAY;
            } else {
        ...
        return (
            (balanceDay0 * currentConversionRate) / RAY,
            pendingDeposits,
            claimable
        );
    }
        ...
        whenNotPaused
    {
        ...
        require(
            _token0.poolBalance >=
                (expectedPoolToken0 * (DENOM - POOL_ERR)) / DENOM,
            "UNEXPECTED_POOL_BALANCES"
        );
        require(
            _token0.poolBalance <=
                (expectedPoolToken0 * (DENOM + POOL_ERR)) / DENOM,
        ...
        require(
            _token1.poolBalance >=
                (expectedPoolToken1 * (DENOM - POOL_ERR)) / DENOM,
            "UNEXPECTED_POOL_BALANCES"
        );
        require(
            _token1.poolBalance <=
        ...
        uint256 token0Floor = _token0Data.reserves +
            (_token0Data.active * token0FloorNum) /
            DENOM;
        uint256 token1Floor = _token1Data.reserves +
            (_token1Data.active * token1FloorNum) /
            DENOM;
        ...
        if (token0Floor > _token0.available) {
            ...
            uint256 token1NeededToSwap = calcAmountIn(
            ...
            uint256 swapAmount = (token1Ceiling + token1NeededToSwap <
                _token1.available)
            ...
            uint256 token0NeededToSwap = calcAmountIn(
        ...
        if (_token0.available > _token0.original) {
            _token0.available -=
                ((_token0.available - _token0.original) * core.protocolFee()) /
                core.MAX_FEE();
        }
        if (_token1.available > _token1.original) {
            _token1.available -=
                ((_token1.available - _token1.original) * core.protocolFee()) /
                core.MAX_FEE();
        }
        ...
        _token0.available =
            _token0.available +
            _token0Data.depositRequestsTotal -
            _token0.newClaimable;
        _token1.available =
            _token1.available +
            _token1Data.depositRequestsTotal -
            _token1.newClaimable;
        ...
        (token0Data.active, token1Data.active) = depositLiquidity(
            _token0.available,
            _token1.available
        );
    ...
    function _assetDataStatics(AssetData storage assetData)
        internal
        view
        returns (AssetDataStatics memory)
    {
    ...
    function getPoolBalances()
        ...
        view
        virtual
        returns (uint256 poolToken0, uint256 poolToken1);
    ...
    function withdrawLiquidity()
        internal
        returns (uint256 token0Withdrawn, uint256 token1Withdrawn)
    {
        ...
        returns (uint256 token0Deposited, uint256 token1Deposited)
    {
        ...
        if ((availableToken0 < (1 gwei)) || (availableToken1 < (1 gwei)))
            return (0, 0);
        (token0Deposited, token1Deposited) = _depositLiquidity(
            availableToken0,
            availableToken1
        );
        _stakeLiquidity();
    ...
    function token0BalanceDay0(address user)
        external
        view
        override
        returns (uint256)
    ...
    function epochToToken0Rate(uint256 _epoch)
        external
        view
        override
        returns (uint256)
    ...
    function token1BalanceDay0(address user)
        external
        view
        override
        returns (uint256)
    ...
    function epochToToken1Rate(uint256 _epoch)
        external
        view
        override
        returns (uint256)
    ...
    function __user_balanceDay0(AssetData storage assetData, address user)
        internal
        view
        returns (uint256)
    {
        ...
        if (depositReq.epoch < epoch) {
            res +=
                (depositReq.amount * RAY) /
                assetData.epochToRate[depositReq.epoch];
        }
        Request memory withdrawReq = assetData.withdrawRequests[user];
    ...
    /// what makes sense for the user
    function __user_requestView(Request memory req)
        internal
        view
        returns (uint256)
    {
    ...
    function feesAccrued()
        ...
        view
        override
        returns (uint256 token0Fees, uint256 token1Fees)
    {
        token0Fees =
    ...
    function setToken0Floor(uint256 _token0FloorNum)
        external
        override
        onlyStrategist
    {
    ...
    function setToken1Floor(uint256 _token1FloorNum)
        external
        override
        onlyStrategist
    {
    ...
    function setEpochDuration(uint256 _epochDuration)
        external
        override
        onlyStrategist
    {
